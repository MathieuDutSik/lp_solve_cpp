...
STATIC void write_lpcomment(void *userhandle, write_modeldata_func write_modeldata, char *string, bool newlinebefore)
{
...
bool __WINAPI write_lpex(lprec *lp, void *userhandle, write_modeldata_func write_modeldata)
{
  int    i, j, b,
         nrows = lp->rows,
         ncols = lp->columns,
  ...
  write_lpcomment(userhandle, write_modeldata, "Objective function", (bool) (ptr != NULL));
  if(is_maxim(lp))
...
bool LP_writefile(lprec *lp, char *filename)
{
  FILE *output = stdout;
  bool ok;

  if (filename != NULL) {
    ok = (bool) ((output = fopen(filename, "w")) != NULL);
    if(!ok)
      return(ok);
  }
  else
...
bool LP_writehandle(lprec *lp, FILE *output)
{
  bool ok;
